...
`.` accessor is used since all functions/variables are exported in an object. this is object is stored in `math_operations`
...
By default Common JS Modules are enabled in NodeJS,  to use ES Modules we need to enable them manually.
...
ES Modules are asynchronous they don't block the execution of the code 
...
**“ES Modules are statically analyzed and loaded before execution.”**
...
While using, just import from `folder` of `index.js` to get all the functions/values form the folder.
...
## Importing NodeJS Internal/Built-In Modules

use the `require()` with path being `"node:Module_name"` or just the module  `Module_name`
