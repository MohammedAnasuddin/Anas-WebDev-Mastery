...
- `PHASE-2`: **POLL**
  ...
  - > **NodeJS non blocking IO**
    > 
    > Node.js is fundamentally designed around a **non-blocking I/O model** to achieve high performance and scalability. This model allows the application to continue executing other tasks while waiting for I/O operations (like file system access or network requests) to complete, rather than halting the entire process. 
    > 
    > How Node.js Achieves Non-Blocking I/O
    > 
    > The non-blocking behavior is powered by a single-threaded **event loop** and the `libuv` library, which uses a thread pool for some background tasks.
