...
use `app.use()` to handle the request, this takes a callback with parameters `(req, res)`.  The callback is known as **request handler**
...
In Express.js, a **route** defines how an application responds to a client request at a specific endpoint (URL path) and with a specific HTTP method (like `GET`, `POST`, etc.). Each route is associated with one or more handler functions that execute when the request matches the specified criteria.
